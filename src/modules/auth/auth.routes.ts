...
const updateProfileSchema = z.object({
  ...
  username: z.string().min(3).max(32).regex(/^[a-zA-Z0-9_]+$/).nullable().optional(),
...
router.post('/refresh', enterpriseRateLimit({ route: 'auth-refresh', windowMs: 15 * 60 * 1000, max: 30, identifierFrom: (req) => req.body?.refreshToken?.slice?.(0, 16) }), validateBody(refreshSchema), async (req, res, next) => {
  try {
    const result = await authService.refreshTokens(req.body.refreshToken, req, req.body.clientId);
    res.json({ success: true, ...result });
  } catch (err) {
    next(err);
  }
...
router.patch('/me', authGuard, validateBody(updateProfileSchema), async (req: AuthenticatedRequest, res, next) => {
  try {
    const user = await authService.updateCurrentUserProfile(req.user!.id, req.body, req);
    res.json({ success: true, user });
  } catch (err) {
    next(err);
  }
...
router.delete('/me/avatar', authGuard, async (req: AuthenticatedRequest, res, next) => {
  try {
...
router.get('/sessions', authGuard, async (req: AuthenticatedRequest, res, next) => {
  try {
    const sessions = await authService.listMyActiveSessions(req.user!.id, req.user!.sid);
...
router.delete('/sessions/:sessionId', authGuard, async (req: AuthenticatedRequest, res, next) => {
  try {
    const params = sessionParamsSchema.parse(req.params);
    const result = await authService.revokeMySession(req.user!.id, params.sessionId, req);
    res.json({ success: true, ...result });
  } catch (err) {
    next(err);
  }
...
router.post('/sessions/logout-others', authGuard, async (req: AuthenticatedRequest, res, next) => {
  try {
    const result = await authService.logoutAllOtherSessions(req.user!.id, req.user!.sid, req);
    res.json({ success: true, ...result });
  } catch (err) {
    next(err);
  }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      res.status(202).json({
        success: true,
        message: 'Account deletion request received. Use the confirmation code to track status.',
        data: {
...
router.post('/deletion/:requestId/cancel', authGuard, async (req: AuthenticatedRequest, res, next) => {
  try {
    const result = await deletionService.cancelDeletionRequestById(req.params.requestId, req);
